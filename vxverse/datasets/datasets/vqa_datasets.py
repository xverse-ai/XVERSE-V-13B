...
from vxverse.datasets.datasets.base_dataset import BaseDataset
...
class OKVQAEvalData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # question = f"Give the following image: ImageContent. Answer the question with a single word or a short phrase simply. Question: {question}"
        question = f"Answer the question with a single word or a short phrase simply. Question: {question}"
        # return image, question, question_id, img_id
        return {"image":image, "question":question, "question_id":question_id, "img_id":img_id}
...
class IconQAEvalData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        answer = data['answer']
        question = f"[vqa] Based on the image, respond to this question with a single word or phrase: {question}"
...
class GQAEvalData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # question = f"[vqa] Based on the image, respond to this question with a single word or phrase: {question}"
        question = f"Based on the image, respond to this question with a single word or phrase: {question}"
        labels = ann["answer"]
        # return image, question, labels
        return {"image": image, "question": question, "label": labels}
